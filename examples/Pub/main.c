...
{
    university_StudentInfo student;
    strcpy(student.name, "张三"); // 学生姓名（中文）
    strcpy(student.pinyin, "Zhang San"); // 学生姓名的拼音
    student.student_id = 12345; // 学号
    student.birthdate = 20000101; // 出生年月

    ucdrBuffer ub;
    uint32_t student_size = university_StudentInfo_size_of_topic(&student, 0);
    uxr_prepare_output_stream(&session, reliable_out, datawriter_id, &ub, student_size);
    university_StudentInfo_serialize_topic(&ub, &student);

    printf("Send student info: %s, pinyin: %s, id: %ld, birthdate: %ld\n", student.name, student.pinyin, student.student_id, student.birthdate);
